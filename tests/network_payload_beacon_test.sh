...
awk -f rules/network_payload_beacon_correlation.awk data/network_events.psv |
awk -F'|' 'BEGIN {OFS="|"}
{
  case_name=""
...
}' "$actual_file" data/network_payload_beacon_expected.psv > "$result_file"

cat "$result_file" | column -s'|' -t

if grep -q '^FAIL|' "$result_file"; then
  echo "NETWORK_PAYLOAD_BEACON_TEST_FAILED"
  exit 1
fi
